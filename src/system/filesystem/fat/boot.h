...
struct fat_bios_parameter_block {
    ...
    /* sectors before first FAT */
    uint8_t fat_count;
    /* almost always 2, sometimes 1, rarely more. */
    uint16_t max_root_entries;
    uint16_t total_sectors;
    /* if 0, check total_sectors_large */
    unsigned char media_descriptor;
    /* 0xF8 is fixed disk. First byte of each FAT should match this. */
    uint16_t sectors_per_fat;
    uint16_t sectors_per_track;
    /* for disks with geometry */
    uint16_t head_count;
    /* for disks with geometry, e.g. 2 for double-sided floppy. */
    uint32_t sectors_before_partition;
    /* zero for non-partitined disk. */
...
struct fat16_extended_bios_parameter_block {
    ...
    /* 0x00 = removable, 0x80 = hard disk */
    unsigned char current_head;
    /* reserved, Windows NT uses bits one and two as flags. */
    unsigned char extended_boot_signature;
    /* 0x29 indicates the following three entries exist. */
    uint32_t serial_number;
    char label[FAT_LABEL_LENGTH];
    /* volume label, padded with spaces (0x20) */
    char fs_type[FAT_FS_TYPE_LENGTH];
    /* padded with spaces (0x20) e.g "FAT16   " */
