...
void uart_init(uint32_t port, UARTConfiguration_t* conf)
{
	...
	uint8_t lcrValue = hal_bitmask_write(port, UART_LCR_REG, UART_LCR_MODE_CONF_B, 8);
	...
	hal_bitmask_write(port, UART_LCR_REG, UART_LCR_MODE_CONF_A, 8);
	...
	hal_bitmask_write(port, UART_TLR_REG, 0x0, 8); // reset RX_FIFO_TRIG_DMA and TX_FIFO_TRIG_DMA to 0
	...
	// 12. Restore LCR Reg
	hal_bitmask_write(port, UART_LCR_REG, lcrValue, 8);

	// Protocol, Baud Rate, and Interrupt Settings
	// 1. Disable UART
	hal_bitmask_write(port, UART_MDR1_REG, 0x7, 8); // Mode-Select to 0x7 (Disable)

	// 2. Switch to config mode B
	hal_bitmask_write(port, UART_LCR_REG, UART_LCR_MODE_CONF_B, 8);

	// 3. Enable access to EFR_REG
	efrValue = hal_get_address_value(port, UART_EFR_REG);
	hal_bitmask_set(port, UART_EFR_REG, UART_EFR_ENHANCED_EN);

	// 4. Switch to register operational mode
	hal_bitmask_write(port, UART_LCR_REG, 0x0, 8);

	// 5. Clear IER_REG
	hal_bitmask_write(port, UART_IER_REG, 0x0, 8);
	...
	hal_bitmask_write(port, UART_DLL_REG, BIT_TRIM_LEFT(divisor, 8), 8); // LSB
	hal_bitmask_write(port, UART_DLH_REG, (divisor >> 8), 8); // MSB

	// 8. Switch to register operational mode
	hal_bitmask_write(port, UART_LCR_REG, 0x00, 8);

	// 9. Load new interrupt configuration
	// no other interrupts are enabled at this time
	hal_bitmask_set(port, UART_IER_REG, BV(0)); // RHR enable
	...
	// set char length
	hal_bitmask_write(port, UART_LCR_REG, hal_get_address_value(port, UART_LCR_REG) | conf->char_length, 8);
